...
void	Response::formResponse(const HttpRequest& request, const Webserv& webserv)
{
	...
	if (1== 2 && (full_path.empty() || !std::filesystem::exists(full_path))) {
		std::cout << request.method << std::endl;
		formError(404, webserv);
	}
	else {
		if (full_path != "./website/download" && full_path != "./website/delete" && request.method == "GET" && isMethodAllowed(request.method)) {
...
void	Response::handleDELETE(const HttpRequest &request, const Webserv &webserv)
{
	CgiHandler handler(request, webserv, "/Users/ashirzad/Desktop/w/website/delete/delete.py");
